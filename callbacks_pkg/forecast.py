...
def _normalize_volume_df(df: pd.DataFrame, cat_hint: Optional[str] = None) -> pd.DataFrame:
    ...
    # optional business area (for BA-level daily/interval)
...
def _numeric_format():
    if Format is None:
        return None
    try:
        if getattr(Scheme, "fixed", None) is not None and getattr(Trim, "yes", None) is not None:
            return Format(precision=1, scheme=Scheme.fixed, trim=Trim.yes)
...
def _ratio_fig(df: pd.DataFrame, title: str):
    ...
    melted["Ratio"] = pd.to_numeric(melted["Ratio"], errors="coerce").round(1)
    melted = melted.dropna(subset=["Ratio"])
    if melted.empty:
        return _empty_fig("No ratio data")
    month_order = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "Avg"]
    melted["Month"] = pd.Categorical(melted["Month"], categories=month_order, ordered=True)
    melted = melted.sort_values(["Year", "Month"])
...
def _format_ratio_wide(df: pd.DataFrame, add_avg: bool = False) -> pd.DataFrame:
    ...
    if add_avg:
        avg_series = numeric.mean(axis=1, skipna=True).round(1)
...
def _smoothing_core(
    ...
    if build_figs:
        ...
        fig_series.update_yaxes(tickformat=".1f")
    else:
...
def _run_volume_summary(n_clicks, data_json, iq_summary_store, phase_store):
    ...
    try:
        ...
        pivot_cols = _cols(piv0) if not piv0.empty else []
        split_cols = _cols(split0) if not split0.empty else []
        ...
        iq_cols = _cols(iq_df) if not iq_df.empty else []
        vol_cols = _cols(vol_df) if not vol_df.empty else []
        ratio_cols = _cols(ratio_df) if not ratio_df.empty else []
...
def _run_prophet_smoothing(n_clicks, seasonality_store, iq_summary_store, cat, holiday_store):
    ...
    try:
        ...
        line_fig.update_yaxes(tickformat=".1f")
        ...
        for col in table_df.columns:
            col_def = {"name": col, "id": col, "editable": col == "Final_Smoothed_Value"}
            try:
...
def _run_phase1_from_volume(n_clicks, prophet_store, phase_store, holiday_store):
    ...
    df = df.dropna(subset=["ds", "Final_Smoothed_Value"]).copy()
    ...
    if df["ds"].duplicated().any():
        ...
        for col in df.columns:
            if col not in numeric_cols and col != "ds":
                agg[col] = "first"
        df = df.groupby("ds", as_index=False).agg(agg)
...
def _run_phase2_from_volume(
    ...
    if basis_norm == "iq":
        ...
        if results_store:
            try:
                ...
                rs_payload = {}
        data_json = rs_payload.get("data") if isinstance(rs_payload, dict) else None
        if data_json:
            try:
                df_norm = pd.read_json(io.StringIO(data_json), orient="split")
            except Exception:
                df_norm = pd.DataFrame()
            df_norm = _normalize_volume_df(df_norm)
            if cat and "category" in df_norm.columns:
                df_norm = df_norm[df_norm["category"] == cat]
            if not df_norm.empty and {"date", "volume"}.issubset(df_norm.columns):
                ...
                df_norm["_month_start"] = df_norm["date"].dt.to_period("M").dt.to_timestamp()
                monthly = df_norm.groupby("_month_start", as_index=False)["volume"].sum()
                monthly = monthly.rename(columns={"_month_start": "ds"})

        if monthly.empty and vol_df is not None and not vol_df.empty and "Year" in vol_df.columns:
            ...
            volume_long["ds"] = pd.to_datetime(
                volume_long["Year"].astype(int).astype(str) + "-" + month_num.astype("Int64").astype(str).str.zfill(2) + "-01",
                errors="coerce",
            )
            monthly = (
        ...
        if monthly.empty:
            return _empty_phase2("No volume history found (need Volume upload, or Volume summary for the selected category).")

        monthly = monthly.dropna(subset=["ds", "volume"]).sort_values("ds")
        monthly["Final_Smoothed_Value"] = (pd.to_numeric(monthly["volume"], errors="coerce") / 1_000_000 * 100.0)
    ...
    if volume_long.empty and basis_norm == "volume" and "ds" in df_smooth.columns:
        tmp = df_smooth.copy()
        tmp["ds"] = pd.to_datetime(tmp["ds"], errors="coerce")
        tmp = tmp.dropna(subset=["ds"])
        if not tmp.empty:
            tmp["year"] = tmp["ds"].dt.year.astype(int)
            tmp["month"] = tmp["ds"].dt.strftime("%b")
            tmp["volume"] = (pd.to_numeric(tmp.get("Final_Smoothed_Value"), errors="coerce") / 100.0 * 1_000_000)
            tmp["volume"] = pd.to_numeric(tmp["volume"], errors="coerce")
            volume_long = tmp.dropna(subset=["year", "month", "volume"])[["year", "month", "volume"]]
    ...
    if basis_norm == "iq":
        ...
        if not wide.empty:
            display_phase2 = wide.copy()
            for col in [c for c in display_phase2.columns if c != "Model"]:
                display_phase2[col] = (
                    pd.to_numeric(display_phase2[col], errors="coerce") * 1_000_000
                ).round().astype("Int64")
        elif not combined.empty:
            display_phase2 = combined.copy()
            if "Forecast" in display_phase2.columns:
                display_phase2["Forecast"] = (
                    pd.to_numeric(display_phase2["Forecast"], errors="coerce") * 1_000_000
                ).round().astype("Int64")
    ...
    edit_cols = [
        ...
        {
            "name": "Volume Split% last 3Months",
            "id": "Vol_Split_Last_3M_Numeric",
            "editable": False,
            "type": "numeric",
            **({"format": fmt} if fmt is not None else {}),
        },
        {
            "name": "Vol_Split_Normalized",
            "id": "Vol_Split_Normalized",
            "editable": True,
            "type": "numeric",
            **({"format": fmt} if fmt is not None else {}),
        },
    ...
    return (
        f"Phase 2 forecast ready ({forecast_months} months, basis: {'IQ' if basis_norm == 'iq' else 'Volume'}).",
...
def _run_forecast(n, n_phase, n_saved, contents, filename, months, phase_store, saved_run_id):
    ...
    try:
        ...
        if not accuracy_tbl.empty:
            ...
            accuracy_fig.update_yaxes(tickformat=".1f")
...
def _di_load_original_data(
    ...
    if fallback_json:
        try:
            ...
            df = pd.DataFrame()
        df = _di_normalize_original_data(df, group_value, group_level)
        if not df.empty:
...
def _di_build_forecasts(
    ...
    orig_df, orig_msg = _di_load_original_data(group_value, group_level, original_data_json)
...
@app.callback(
    ...
    Input("di-group-level", "value"),
    State("vs-data-store", "data"),
    prevent_initial_call=True,
)
def _di_load_transform(saved_run_id, group_level, vs_data_json):
    if not saved_run_id:
        raise dash.exceptions.PreventUpdate
    ...
    group_level_norm = str(group_level or "forecast_group").strip().lower()
    df_use = df.copy()
    df_cols = {c.lower(): c for c in df_use.columns}
    fg_col = df_cols.get("forecast_group") or df_cols.get("queue_name") or df_cols.get("category")
    model_col = df_cols.get("model")

    if group_level_norm == "business_area":
        ba_col = df_cols.get("business_area")
        if not ba_col and vs_data_json and fg_col:
            try:
                orig = pd.read_json(io.StringIO(vs_data_json), orient="split")
            except Exception:
                orig = pd.DataFrame()
            orig_norm = _normalize_volume_df(orig)
            if not orig_norm.empty and {"forecast_group", "business_area"}.issubset(orig_norm.columns):
                mapping_df = (
                    orig_norm.dropna(subset=["forecast_group", "business_area"])
                    .groupby("forecast_group")["business_area"]
                    .agg(lambda s: s.mode().iat[0] if not s.mode().empty else s.iloc[0])
                    .reset_index()
                )
                if not mapping_df.empty:
                    if fg_col == "forecast_group":
                        df_use = df_use.merge(mapping_df, on="forecast_group", how="left")
                    else:
                        df_use = df_use.merge(mapping_df, left_on=fg_col, right_on="forecast_group", how="left")
                        if "forecast_group" in df_use.columns and fg_col in df_use.columns:
                            df_use = df_use.drop(columns=["forecast_group"])
                    ba_col = "business_area"

        if ba_col:
            if ba_col != "business_area":
                df_use = df_use.rename(columns={ba_col: "business_area"})
            df_cols = {c.lower(): c for c in df_use.columns}
            model_col = df_cols.get("model")

            group_keys = ["business_area"]
            if "Month_Year" in df_use.columns:
                group_keys.append("Month_Year")
            if "Year" in df_use.columns:
                group_keys.append("Year")
            if "Month" in df_use.columns:
                group_keys.append("Month")
            if model_col:
                group_keys.append(model_col)
            ...
            sum_cols = [c for c in value_candidates if c in df_use.columns]
            sum_cols.extend(
                [
                    c
                    for c in df_use.columns
                    if str(c).startswith("Forecast_") and c not in sum_cols
                ]
            )
            sum_cols = [c for c in sum_cols if c not in group_keys]
            for col in sum_cols:
                df_use[col] = pd.to_numeric(df_use[col], errors="coerce")
            if sum_cols:
                df_use = (
                    df_use.groupby(group_keys, as_index=False)[sum_cols]
                    .sum(min_count=1)
                )
            msg = f"{msg} | Rolled up to Business Area."
        else:
            msg = f"{msg} | Business Area not found (need Business_Area in original data to roll up)."
            group_level_norm = "forecast_group"

    df_cols = {c.lower(): c for c in df_use.columns}
    group_col = df_cols.get("business_area") if group_level_norm == "business_area" else (
        df_cols.get("forecast_group") or df_cols.get("queue_name") or df_cols.get("category")
    )
    model_col = df_cols.get("model")
    group_opts = _opts(group_col) if group_col else []
    ...
    if "Month_Year" in df_use.columns:
        df_use["Month_Year"] = df_use["Month_Year"].astype(str)
        month_vals = _di_sort_month_str(df_use["Month_Year"].dropna().astype(str).unique().tolist())
        month_opts = [{"label": m, "value": m} for m in month_vals]
        month_val = month_opts[0]["value"] if month_opts else None
    else:
        month_col = df_cols.get("month")
        year_col = df_cols.get("year")
        if month_col and year_col:
            df_use["_month_year"] = df_use[month_col].astype(str).str[:3] + " " + df_use[year_col].astype(str)
            month_vals = _di_sort_month_str(df_use["_month_year"].dropna().astype(str).unique().tolist())
            month_opts = [{"label": m, "value": m} for m in month_vals]
            month_val = month_opts[0]["value"] if month_opts else None

    preview = df_use.head(200)
    return (
        msg,
        ...
        month_opts,
        month_val,
        df_use.to_json(date_format="iso", orient="split"),
    )
...
def _di_run_interval_forecast(
    ...
    if iv.empty:
        orig_df, _ = _di_load_original_data(group_value, group_level, vs_data_json)
...
def _di_save_results(n, results_json):
    ...
    try:
        ...
        return f"Save failed: {exc}", shown_style
